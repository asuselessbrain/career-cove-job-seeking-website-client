...
const MyJobs = () => {
    ...
    const fetchData = async () => {
        try {
            const { data } = await axios(`https://job-seeking-server-alpha.vercel.app/job/${user?.email}`);
            setJobs(data);
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        try {
            // Send a DELETE request to your backend API to delete the job
            const { data } = await axios.delete(`https://job-seeking-server-alpha.vercel.app/job/${id}`);
    ...
    return (
        <section className="container px-4 min-h-[calc(100vh-470px)] mx-auto my-10">
            <div className="flex items-center gap-x-3">
                <h2 className="text-lg font-medium text-gray-800 dark:text-white">Total Added Job</h2>

                <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">{jobs.length} Jobs</span>
            </div>
                    ...
                    <div className='inline-block min-w-full py-2 align-middle md:px-6 lg:px-8'>
                        <div className='overflow-hidden border border-gray-200  md:rounded-lg'>
                           ...
                           text-xs'
                                                ...
                                                <td
                                                    title={job.description} // Set the full description as the title for tooltip
